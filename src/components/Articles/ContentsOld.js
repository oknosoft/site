...
class Contents extends Component {
  ...
  shouldComponentUpdate({title, match, location, docRef}) {
    let needUpdate = true;
    if(!docRef) {
      docRef = location.pathname.replace(match.path, '').replace(/^\/|\/$/g, '');
    }
    // if(title != ltitle) {
    //   handleIfaceState({
    //     component: '',
  ...
  readContents(id) {
    const rows = [];
    const keys = [];
    const {cat: {articles}, utils} = $p;
    articles.find_rows({published: true}, (row) => {
